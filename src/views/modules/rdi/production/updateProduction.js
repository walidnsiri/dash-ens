...
const UpdateProduction = (props) => {
  ...
  const [selectedBadge, setSelectedBadge] = useState(rdi?.refproduction_id);
  ...
  const initialValues = {
    production: rdi? rdi.production:"",
    description: rdi? rdi.description:"",
    refproduction: rdi? rdi.refproduction_id:"",
    charge: rdi? rdi.charge_h:"",
    dateproduction: new Date(rdi.date_production),
  }
  ...
  const updaterdi = async function (values) {
    ...
    const [user, error] = await queryApi("rdi/"+rdi.id, body, 'PUT');
    if (user) {
      setModal({ show: true, message: "L'rdi a été modifié avec succès", type: 'success' });
    }
