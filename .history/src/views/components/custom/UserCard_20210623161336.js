...
const UserCard = (props) => {
  ...
  const [img,setImg] = useState({loading: false, data: noimage});

  useEffect(() => {
    async function loadimg() {
      if(user.image) {
      setImg({loading: true, data : noimage});
      const img = await fetchImageFromService(user.image);
      setImg({loading: false, data : img});
      console.log(img.data)
      }
      else {
        setImg({loading: false, data : noimage});
      }
    }
    loadimg();
  }, [user.image]);
  ...
  return (
    <>
      ...
      {img.loading ? <LoaderLarge/> : <CImg src={img.data}/> }
        </div>
