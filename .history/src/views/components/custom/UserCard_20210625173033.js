...
const UserCard = (props) => {
  ...
  const [img,setImg] = useState(noimage , async () => {
    return fetchImageFromService(user.image).then( img => {return img});
  });

  /*useEffect(() => {
    (async () => {
      let ImgPromise = fetchImageFromService(user.image);
        Promise.resolve(ImgPromise).then(img => setImg(img)); 
    })();
  }, [user.image]);*/
    ...
    <>
      ...
      <CImg src={img}/> 
        ...
        <div className="post-content">
          ...
          {<h2 className="sub_title">{user.authorities[0].authority}</h2>}
          <div
