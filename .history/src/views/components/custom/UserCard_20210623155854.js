...
const UserCard = (props) => {
  ...
  const [img,setImg] = useState({loading:true, data: null});

  useEffect(() => {
    async function loadimg() {
      setImg({loading: true, data : null});
      if(user.image) {
      const img = await fetchImageFromService(user.image);
      setImg({loading: false, data : img});
      }
    }
    loadimg();
  }, [user.image]);
  ...
  return (
    <>
      ...
      {img.loading && <LoaderLarge/>}
      {(!img.loading && img.data) ?
        <CImg src={img.data} alt=""/>
        :
        <CImg src={noimage} alt="profile_img"/>
      }
        </div>
