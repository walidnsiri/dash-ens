...
function Card(setUser) {
  ...
  const handleLogin = function(e) {
    let body = JSON.stringify({
      ...
      }).then(response => response.text().then(user => {setUser.setUser(user); history.replace("dashboard");console.log("im in!!")}) ).catch( err => console.log(err));
     */
    let user,error= null;
    [user,error] = queryApi("/public/login",body,"POST");
    if(user != null) {setUser.setUser(user);history.replace("dashboard");} 
