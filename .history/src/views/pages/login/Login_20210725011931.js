...
function Card(setUser) {
  ...
  const handleReintialiserEmail = async function (values) {
    ...
    const [msg, error] = await queryApi("/public/forgottenpassword/"+values.email);
    if (msg) {
      setRenderEmailError(msg);
    } 
    if(error) {
      setRenderEmailError("Veuillez consulter votre email pour terminer la réinitialisation.")
    }
  };
